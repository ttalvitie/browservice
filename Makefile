...
CFLAGS_release := $(CFLAGS_COMMON) -DNDEBUG -O3 -DNDEBUG -fdata-sections -ffunction-sections -fno-ident -U_FORTIFY_SOURCE -D_FORTIFY_SOURCE=2
LDFLAGS_COMMON := -rdynamic -fPIC -pthread -Wl,--disable-new-dtags -Wl,--fatal-warnings -Wl,-rpath,. -Wl,-z,noexecstack -Wl,-z,now -Wl,-z,relro -Wl,-rpath,"\$$$$ORIGIN" cef/Release/libcef.so -lPocoFoundation -lPocoNet `pkg-config --libs pangoft2` -ljpeg
...
define OUTDEFS
OBJS_$(1) := $(SRCS:src/%.cpp=$(1)/obj/%.o)
DEPS_$(1) := $(SRCS:src/%.cpp=$(1)/obj/%.d)
...
define OBJRULE
$(2:src/%.cpp=$(1)/obj/%.o): $(2) cef/include
	@mkdir -p $(1)/obj
	$(CXX) $(CFLAGS_$(1)) -Icef -Igen -MMD -c $(2) -o $(2:src/%.cpp=$(1)/obj/%.o)
endef
